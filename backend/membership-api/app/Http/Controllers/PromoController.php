...
class PromoController extends Controller
{
    ...
    private function validated(Request $r): array { $data = $r->validate(['title'=>'required|string|max:255','image'=>'nullable|image|max:3072','description'=>'required|string','terms'=>'nullable|string','discount_type'=>['required',Rule::in(['percentage','fixed'])],'discount_value'=>'required|numeric|gt:0','start_date'=>'required|date','end_date'=>'required|date|after_or_equal:start_date','status'=>['required',Rule::in(['active','inactive'])]]); if ($data['discount_type'] === 'percentage' && (float) $data['discount_value'] > 100) abort(422, 'Diskon persentase tidak boleh lebih dari 100%.'); if ($r->hasFile('image')) $data['image_path'] = $r->file('image')->store('promos','public'); unset($data['image']); return $data; }
}
